...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base,
	...
	case MQTT_EVENT_CONNECTED:
		...
		msg_id = esp_mqtt_client_subscribe(client, "/stu_040/lamp1", 0);
		ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

		msg_id = esp_mqtt_client_subscribe(client, "/stu_040/lamp2", 0);
		ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

		msg_id = esp_mqtt_client_subscribe(client, "/stu_040/lamp3", 0);
		ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

		msg_id = esp_mqtt_client_subscribe(client, "/stu_040/lamp4", 0);
		ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

		msg_id = esp_mqtt_client_subscribe(client, "/stu_040/lamp5", 0);
		ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

		msg_id = esp_mqtt_client_subscribe(client, "/stu_040/lamp6", 0);
		ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

		msg_id = esp_mqtt_client_subscribe(client, "/stu_040/lamp7", 0);
		ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);

		msg_id = esp_mqtt_client_subscribe(client, "/stu_040/lamp8", 0);
		ESP_LOGI(TAG, "sent subscribe successful, msg_id=%d", msg_id);
		break;
	...
	case MQTT_EVENT_DATA:
		...
		if (strncmp(event->topic, "/stu_040/lamp1", event->topic_len) == 0) // if topic is "/stu_999/lamp1" then result = 0
				{
			ESP_LOGI(TAG, "event->topic = /stu_040/lamp1");
		...
		if (strncmp(event->topic, "/stu_040/lamp2", event->topic_len) == 0) // if topic is "/stu_999/lamp1" then result = 0
				{
			ESP_LOGI(TAG, "event->topic = /stu_040/lamp2");
			if (strncmp(event->data, "1", event->data_len) == 0) // if data is "1" then result = 0
					{
				ESP_LOGI(TAG, "Turn on LED");
				gpio_set_level(26, 1);
		...
		if (strncmp(event->topic, "/stu_040/lamp3", event->topic_len) == 0) // if topic is "/stu_999/lamp1" then result = 0
				{
			ESP_LOGI(TAG, "event->topic = /stu_040/lamp3");
			if (strncmp(event->data, "1", event->data_len) == 0) // if data is "1" then result = 0
					{
				ESP_LOGI(TAG, "Turn on LED");
				gpio_set_level(27, 1);
		...
		if (strncmp(event->topic, "/stu_040/lamp4", event->topic_len) == 0) // if topic is "/stu_999/lamp1" then result = 0
				{
			ESP_LOGI(TAG, "event->topic = /stu_040/lamp4");
			if (strncmp(event->data, "1", event->data_len) == 0) // if data is "1" then result = 0
					{
				ESP_LOGI(TAG, "Turn on LED");
				gpio_set_level(14, 1);
		...
		if (strncmp(event->topic, "/stu_040/lamp5", event->topic_len) == 0) // if topic is "/stu_999/lamp1" then result = 0
				{
			ESP_LOGI(TAG, "event->topic = /stu_040/lamp5");
			if (strncmp(event->data, "1", event->data_len) == 0) // if data is "1" then result = 0
					{
				ESP_LOGI(TAG, "Turn on LED");
				gpio_set_level(12, 1);
		...
		if (strncmp(event->topic, "/stu_040/lamp6", event->topic_len) == 0) // if topic is "/stu_999/lamp1" then result = 0
				{
			ESP_LOGI(TAG, "event->topic = /stu_040/lamp6");
			if (strncmp(event->data, "1", event->data_len) == 0) // if data is "1" then result = 0
					{
				ESP_LOGI(TAG, "Turn on LED");
				gpio_set_level(32, 1);
		...
		if (strncmp(event->topic, "/stu_040/lamp7", event->topic_len) == 0) // if topic is "/stu_999/lamp1" then result = 0
				{
			ESP_LOGI(TAG, "event->topic = /stu_040/lamp7");
			if (strncmp(event->data, "1", event->data_len) == 0) // if data is "1" then result = 0
					{
				ESP_LOGI(TAG, "Turn on LED");
				gpio_set_level(33, 1);
		...
		if (strncmp(event->topic, "/stu_040/lamp8", event->topic_len) == 0) // if topic is "/stu_999/lamp1" then result = 0
				{
			ESP_LOGI(TAG, "event->topic = /stu_040/lamp8");
			if (strncmp(event->data, "1", event->data_len) == 0) // if data is "1" then result = 0
					{
				ESP_LOGI(TAG, "Turn on LED");
				gpio_set_level(25, 1);
